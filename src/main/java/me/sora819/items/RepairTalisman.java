...
public class RepairTalisman{
	...
	public ItemStack getItem() {
		...
		meta.setDisplayName(ChatColor.translateAlternateColorCodes('&', core.configManager.getString("repair_item_name")));
		List<String> list = new ArrayList<String>();
		
		for (String str : (List<String>)core.configManager.getList("repair_item_lore")) {
			list.add(ChatColor.translateAlternateColorCodes('&', str));
		}
		meta.setLore(list);
		item.setItemMeta(meta);

		main.nbth.setNBTString(item,"special","repair");

		return item;
	}
	
	public void addRecipes() {
		core.recipeManager.addRecipe(main, core.recipeManager.getSpecialItemRecipe(main, "repair", getItem()));
	}
	...
	public boolean itemCheck_old(ItemStack item) {
		if(item != null) {
			if(item.hasItemMeta() && item.getItemMeta().hasLore()) {
				for(int i = 0; i<core.configManager.getList("repair_item_lore").size(); i++) {
	...
	public boolean itemCheck(ItemStack item){
		...
		if(main.nbth.getNBTString(item, "special").equals("repair")){
			return true;
		}
		return false;
	}
        ...
        scheduler.scheduleSyncRepeatingTask(main, new Runnable() {
            ...
            public void run() {
            	...
            	for (Player ply : players) {
        			...
        			if(!Boolean.parseBoolean(core.configManager.getString("repair_stackable"))) {
        				stack = 1;
        			}

    				for(int i=0; i<stack; i++) {
						core.invManager.repairAll(ply, core.configManager.getInt("repair_amount"));
    				}
            	}
        	}
        }, 0L, Integer.toUnsignedLong(core.configManager.getInt("repair_time")));
	}
}
